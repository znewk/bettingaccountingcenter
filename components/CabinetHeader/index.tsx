...
import { useUserStore } from '../../store/useUserStore'; // Используйте ваш zustand стор
import { menuItems } from './data';

const CabinetHeader: React.FC<{ title: string }> = ({ title }) => {
  const [visible, setVisible] = useState(false);
  const userName = useUserStore(state => state.name); // Получаем имя пользователя из zustand стора
